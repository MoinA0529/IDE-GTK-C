...
int main(int argc, char *argv[])
{
    ...
    // memory allocation to the widgets structure
    ...
    // In gtk_builder_connect_signals widgets as 2nd argument is passed which is our structure
    // containing all the necessary gtk widgets. Instead of NULL pointer.
    ...
    //free up the memory allocated to slice/widgets structre
    return 0;
}

// Called when either "Heads" or "Tails" radio button clicked
...
void on_btn_toss_clicked(GtkButton *button, app_widgets *wdgts)
{
    ...
    // Win and Lose text strings and settings, we can change label color, font in GTK here
    ...
    if (result == TRUE)
    {
        ...
        // When result is 'TRUE', 'Heads' is displayed
    }
    else
    {
        gtk_label_set_text(GTK_LABEL(wdgts->w_lbl_result), "Tails");
        // When result is 'False', 'Tails' is displayed
    ...
    else // all the other cases are handled
    {
        // Player lost the game
        gtk_label_set_markup (GTK_LABEL (wdgts->w_lbl_win_lose), str_lose);
    }

    // The following commands are followed after the result is declared
...
void on_btn_play_again_clicked(GtkButton *button, app_widgets *wdgts) // RESET Function
{
    ...
    gtk_label_set_text(GTK_LABEL(wdgts->w_lbl_result), "...");
    gtk_label_set_text(GTK_LABEL(wdgts->w_lbl_win_lose), "If your placed bet is same as the coin toss result, you WIN!!");
